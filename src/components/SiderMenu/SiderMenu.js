...
const getIcon = icon => {
  ...
  if (typeof icon === 'string') {
    return <i className={`${iconfont.iconfont} ${iconfont[icon]} `} style={{marginRight:10}} />;
  }
  return icon;
};
...
class SiderMenu extends PureComponent {
  ...
  render() {
    const { logo, collapsed, onCollapse,isEvent } = this.props;
    ...
    return (
      <div className={styles.main} style={{background:'#333'}}>
         <Sider
        trigger={null}
        collapsible
        collapsed={collapsed}
        ...
        style={{minHeight:window.innerHeight-64}}
      >
        <Menu
          key="Menu"
          theme="dark"
